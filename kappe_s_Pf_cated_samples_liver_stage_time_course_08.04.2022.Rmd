...
```{r}
day2_go_clean <- read.csv("GO_enrichment/day2/day2_vs_spz_ClusterProfiler_AllOntology.csv") %>% 
...
```{r}
cluster_go_dfs <-  read.csv("GO_enrichment/timecourse/time_course_clusters_clusterProfiler_allOntology.csv") %>% 
  filter(qvalue < 0.2) %>% 
...
quantile(top_paths$setSize, probs = seq(0,1, length.out=11))
```
...
```{r}
cluster_go_combined_dfs <- read.csv("GO_enrichment/timecourse/time_course_clusters_combinedProfiles_clusterProfiler_allOntology.csv") %>% 
    mutate(labels=case_when(
    grepl("RNA polymerase", go_term_name) ~ gsub("RNA polymerase","RNApol",go_term_name),
    grepl(".apurinic or apyrimidinic site.", go_term_name) ~ gsub(".apurinic or apyrimidinic site.","", go_term_name),
    grepl(",", go_term_name) ~ gsub("^(.+),.+","\\1",go_term_name),
...
range(top_paths_profiles$neg_log10_p)
```
...
bubble_plot_profile <- ggplot(top_paths_profiles, aes(x=neg_log10_p, y=ID)) +
      ...
      guides(fill="none",
             size=guide_legend(title = "Number of Genes", size = 1)) +
      theme_classic() +
...
colnames(ap2_mat_rename) <- gsub("Pf_Day0","sporozoites",colnames(expn_mat)) %>% 
  gsub("^.+_(Day[0-9])(_R.+)","\\1_Pf_LS\\2", .)
...
gene_anno <- select(ap2_expn, gene_id=new_rownames, gene_name_or_symbol) %>% 
  distinct() %>% 
  mutate(genome="PF3D7") %>% 
  as.data.frame() %>% 
  set_rownames(.$new_rownames)

s_ord <- c("sporozoites_Rep1", "sporozoites_Rep2", "sporozoites_Rep3", "sporozoites_Rep4", "sporozoites_Rep5", 
...
# png(paste0("figures/combat_seq/AP2_expression/AP2_heatmap_row_zscores_heatmap.png"),units="in", height = 10, width = 15, res=150)
# svglite(paste0("figures/combat_seq/AP2_expression/AP2_heatmap_row_zscores_heatmap.svg"),fix_text_size=FALSE, system_fonts = list(sans = "Open Sans"), height = 10, width = 15)
...
day2_sig <- day2_AP2_results %>% 
  ...
  filter(any(negLog10P >= 1.2)) %>% 
  ungroup() %>% 
  mutate(log2enr_clean=ifelse(negLog10P >= 1.2, log2enr, NA)) %>% 
...
# write.csv(cluster_hits_enr, "motif_analysis/monaLisa/time_course_ap2_withTranscriptIDs_results.csv", row.names = FALSE)
```
...
# write.csv(profile_hits_enr, "motif_analysis/monaLisa/time_course_profiles_ap2_withTranscriptIDs_results.csv", row.names = FALSE)
```
...
```{r}
with_prom_bg <- read.csv("motif_analysis/monaLisa/time_course_ap2_results.csv")
profiles_with_prom_bg <- read.csv("motif_analysis/monaLisa/time_course_profiles_ap2_results.csv")
```
...
```{r}
orthoGroups_species <- read.delim("motif_analysis/orthogroups_pf_to_all_species_v2.txt", sep = "\t") %>% 
  janitor::clean_names() %>% 
...
```
Rscript motif_analysis/uniprot.ws_lookup.R
```

```{r}
ortholog_search_res <- dir("motif_analysis/uniprot_ids",
                           pattern = "*.RDS", 
                           full.names = TRUE)
...
length(pwm) #1956
length(pfm) #1956
# saveRDS(pwm, "motif_analysis/JASPAR2022_PWMs.RDS")
# saveRDS(pfm, "motif_analysis/JASPAR2022_PFMs.RDS")
```

```{r}
all_PWMs <- readRDS("motif_analysis/JASPAR2022_PWMs.RDS")
all_PFMs <- readRDS("motif_analysis/JASPAR2022_PFMs.RDS")
...
# write.csv(sel_motif_annots, "motif_analysis/jaspar2022_orthologs.csv", row.names = F)
```
...
# saveRDS(ortho_day2_enr,"motif_analysis/monaLisa/day2_vs_spz_orthologs_monaLisa_results.RDS")
```
...
# dim(ortho_enr_df)
ortho_enr_df
# write.csv(ortho_enr_df,"motif_analysis/monaLisa/day2_vs_spz_orthologs_monaLisa_results.csv", row.names = FALSE)
```
...
# saveRDS(ortho_cluster_motifs,"motif_analysis/monaLisa/time_course_ortholog_results_list.RDS")
```
...
# write.csv(ortho_cluster_motifs_dfs,"motif_analysis/monaLisa/time_course_orthologs_results.csv", row.names = FALSE)
...
# saveRDS(ortho_profile_motifs, "motif_analysis/monaLisa/time_course_profiles_orthologs_results_list.RDS")
```
...
# write.csv(ortho_profile_motifs_dfs,"motif_analysis/monaLisa/time_course_profiles_orthologs_results.csv", row.names = FALSE)
...
```{r}
ortho_cluster_motifs_dfs <- read.csv("motif_analysis/monaLisa/time_course_orthologs_results.csv")
...
# head(input)
# length(unique(input$motif))
```

```{r}
sel_motif_annots <- read.csv("motif_analysis/jaspar2022_orthologs.csv")
...
unique(summary_stats$motif)[order(unique(summary_stats$motif))]
unique(input$motif)[order(unique(input$motif))]
...
# svglite("figures/motif_analysis/time_point_cluster_ortholog_heatmap_seqlogos_tileplot_rows.svg", height = 12, width=10,
#         system_fonts = list(sans = "Open Sans"),fix_text_size=FALSE)

# svg("figures/motif_analysis/time_point_cluster_ortholog_heatmap_seqlogos_tileplot_rows.svg", height = 12, width=12)
...
```{r fig.height=10, fig.width=10}
# svg("figures/motif_analysis/time_point_cluster_ortholog_TF_expn_heatmap_seqlogos_tileplot_rows.svg", height = 12, width=10)
...
```{r}
ortho_profile_motifs_dfs <- read.csv("motif_analysis/monaLisa/time_course_profiles_orthologs_results.csv")
