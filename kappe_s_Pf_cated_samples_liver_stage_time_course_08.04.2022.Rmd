...
ppi_modules <- function(time_course_res, cluster_name, input_network, msp_fit, DEGs = NULL, percentile=0.1){
  ...
  # make the initial subnetwork composed only of proteins found in the PPI 
  # if( any(grepl("matrix",class(pvalues))) ) {
  #   time_course_genes <-  rownames(pvalues)
  # }else{
  #   time_course_genes <- names(pvalues)
  # }
  ...
  # Add scores to the subnetworks
   ...
   cutoff <- quantile(fb$pvalues, probs = percentile)
...
ppi_network_figures <- function(input_module, protein_complex_annots, DEGs, title = ""){
    ...
    protein_complexes <- protein_complexes %>% 
        ...
        mutate(node_color = case_when(
           ...
           protein_complex_id == "unassigned" ~ "black",
    ...
    if(all(c(chk2, chk3, chk4))){
     ...
     # plot using ggraph
      ...
      pt_size <- ifelse(length(sel_nodes) <= 50, 15, 8)
      ...
      module_graph <- ggraph(graph_tbl, 
        ...
        # figs <- list("module_graph" = module_graph,
        #               "fc_plot" = fc_plot,
        #               "protein_complex_anno" = protein_complex_anno,
        #               "ppic_anno" = ppic_anno)
...
```{r}
sel_modules1 <- lapply(up_down_ppi_modules, function(x) x[["module"]])

sapply(names(sel_modules1), function(x){
  p <- time_course %>% 
    filter(combined_gene_clusters == x) 
  
  table(nodes(sel_modules1[[x]]) %in% p$gene_id)
})
...
```{r}
d <- degree(graph_from_graphnel(up_down_ppi_modules$Profile3$module)) 
length(d)
quantile(d)
d[d>3] %>% length()
```
...
module_figs <- purrr::map(names(up_down_ppi_modules) %>% set_names(., value = .), function(x){
      ...
      pdf(glue("figures/combat_seq/networks_ppi/time_course_{profile}_sel_module_genes_network_fcs.pdf"), height = 10, width = 15)
      print(fig)
      dev.off()
...
# names(module_figs)
# module_figs$Profile1
# module_figs$Profile3
```
