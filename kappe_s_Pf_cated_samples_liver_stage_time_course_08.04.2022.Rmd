...
design_input <- colData(cb_se) %>%
  ...
  dplyr::select(sample_id_cat, 
                ...
                Time=time_point) %>% 
  # remove Day2_Heps_S3_Pf (L3Pf2_S2xL3Pf2Mix_S3) due to very low read coverage with <500 reads total
...
<<<<<<< HEAD
```{r}
BiocManager::install("CBNplot", update = FALSE, verbose = TRUE)
# library(CBNplot)
```


=======
>>>>>>> 6fa561180c1a5c5b25eeb467e91e41c29e0e9e4c
...
# suppressPackageStartupMessages(library(qpgraph))
# library(CBNplot) 
# library(RTN)
...
# Need gene_names info and clean up bad text encoding from original GTF file being read-in (% symbols)
# make gene_names unique using gene_id where needed
pf_genes <- as.data.frame(rowData(cb_se)) %>% 
  mutate(gene_name = gsub("Name=","", gene_name),
         description = gsub("description=", "", description)) %>% 
  mutate_at(vars(description), ~gsub("\\%2C", " ", .)) %>% 
  # fix ap2 gene names and make gene_names unique
  left_join(., select(ap2_ids, gene_id, old_id = input_id),
            by = "gene_id") %>% 
  group_by(gene_name) %>% 
  mutate_at(vars(gene_name), ~case_when(
    any(duplicated(gene_name)) & gene_name == "ApiAP2" & !is.na(old_id) ~ glue("{gene_name} ({old_id})"),
    any(duplicated(gene_name)) ~ glue("{gene_name} ({gene_id})"),
    TRUE ~ gene_name)) %>% 
  ungroup()
...
DEGs_df <- purrr::map(combat_de_nofilter[grep("Day[456]", names(combat_de_nofilter))], function(res){
...
# DEGs_df$num_upreg %>% table()
#    0    1    2    3 
# 2087  292  602 2207 

# quantile(DEGs_df$mean_log_fc)
#          0%         25%         50%         75%        100% 
# -14.9204182  -2.2697274   0.1963065   2.7945961  12.4804498
```
...
PPI_clusters <- openxlsx::read.xlsx("resources/PPI/1-s2.0-S221112471930909X-mmc7.xlsx") %>% 
  ...
  mutate_at(vars(corum_complex_clean), ~gsub("^;|;\\s{1,}$", "", .) %>% 
              ...
              gsub("al subunit", "e", .) %>% 
              gsub("polymerase", "pol", .) %>% 
              gsub("CCT micro-complex;  CCT complex", "CCT complex", .)) %>% 
...
# table(PPI_clusters$corum_complex_clean)
```
...
protein_complex_annots <- PPI %>% 
  ...
  mutate_at(vars(go_processes,corum_complex_clean), ~ifelse(is.na(.), "", .)) %>% 
  mutate(temporal_expression_changes = ifelse(gene_id %in% time_course$gene_id, "yes", "no")) %>% 
  select(gene_id, 
         temporal_expression_changes,
         protein_complex_id, 
         number_proteins_in_complex,
         everything())


head(protein_complex_annots)
# dim(protein_complex_annots) #2141    
``` 
...
```{r}
ppi_modules <- function(time_course_res, cluster_name, input_network, msp_fit, DEGs = NULL, fdr_percentile=0.1, fdr = NULL){
  ...
  if(!is.null(DEGs)){
      ...
      chk1 <- !any(rownames(input_pf_vals) %in% rownames(pvalues)) # TRUE
      chk2 <- !any(rownames(input_pf_vals) %in% time_course_res$gene_id) #TRUE
      chk3 <- identical(colnames(input_pf_vals), colnames(pvalues)) #TRUE
      # testit::assert("", )
      if(all(c(chk1,chk2,chk3))){
         in_pvalues <- rbind(pvalues, input_pf_vals)
  ...
  n_ppi <- length(nodes(subnet))
  # subnet <- largestComp(subnet)
  print(glue("there are {n_ppi} nodes in the subnetwork."))

  # Add scores to the subnetworks
   if(!is.null(fdr)){
     cutoff <- fdr
     message(glue("Add scores to the subnetworks using the {cutoff} fdr threshold"))
   }else{
     cutoff <- quantile(fb$pvalues, probs = fdr_percentile) 
     message(glue("Add scores to the subnetworks using the top {fdr_percentile} percentile threshold"))
   }
   print(glue("the FDR threshold is {cutoff}"))
...
ppi_network_figures <- function(input_module, protein_complex_annots, DEGs, title = ""){
    ...
    if(!chk1){
      ...
      message("second round of filtering annotations")
      ...
      protein_complexes <- protein_complexes %>% 
        ...
        ungroup() %>% 
        filter(keep) %>% 
        # 2nd check for any remaining duplicate annotations
        mutate(dups = duplicated(gene_id) | duplicated(gene_id, fromLast = TRUE))
      
    }
    
    # merge in DE results and clean-up annots
    protein_complexes <- protein_complexes %>% 
        # recount the number of genes in each PPI complex
        group_by(protein_complex_id) %>%
        mutate(number_genes_in_ppic = n()) %>% 
        ungroup()  %>%
      
        # retain annots for graph visualization
        mutate(hillier_protein_complex_id = protein_complex_id,
               protein_complex_id = case_when(
                          # protein_complex_id == "unassigned" ~ "other", 
                          number_genes_in_ppic == 1 ~ "other", 
                          TRUE ~ protein_complex_id) %>% 
                      as.factor(.)
        ) %>% 
        # define which genes are in protien complexes 
        mutate(in_ppic = ifelse(grepl("other|unassigned", protein_complex_id), "no", "yes")) %>% 
    
        # update color codes for the graph figure
        group_by(protein_complex_id) %>%
        mutate(node_color = case_when(
           protein_complex_id == "other" ~ "black",
           protein_complex_id == "unassigned" ~ "grey80",
           TRUE ~ node_color
        )) %>%
        ungroup() %>%
    
        # Merge in FC data 
        left_join(., DEGs, by = "gene_id") %>% 
    ...
    # ensure there are no duplicates and no genes have been filtered out incorrectly
    ...
    if(all(c(chk2, chk3, chk4))){
        ...
        fc_color <- ifelse(all(fcs$log_fc < 1), "white", "black")
        fc_plot <- ggplot(fcs, aes(x = time_point, y = gene_name, fill = log_fc)) +
          labs(y = "", x = "", title = "Fold-Changes per LS Timepoint") +
          geom_tile() + 
          geom_text(aes(label = round(log_fc, digits = 1)), 
                    color = fc_color,
                    size = 3) +
          # coord_fixed() +
          scale_x_discrete(expand = c(0,0)) +
          scale_fill_gradientn(colours = viridis::plasma(299),
                             limits = c(-10, 10),
                             oob = scales::squish,
                             guide = NULL) +
          theme_classic() +
        ...
        protein_complex_anno <- ggplot(fcs, aes(x = "protein_complex",
                                                y = gene_name)) +
          geom_text(aes(label = protein_complex_id, 
                        color = protein_complex_id),
                    size = 4) +
          scale_color_manual(values = pull(pcid, node_color, 
                                           name = protein_complex_id),
                             guide = NULL) +
          theme_void() +
          theme(plot.title = element_text(size = 8, hjust = 0.5))
        
        anno_df <- fcs %>% 
          select(gene_name, protein_complex_id,
                 go_processes, corum_complex_clean) %>% 
        ...
        ppic_anno <- ggplot(anno_df, aes(x = "protein_complex",
                                         y = gene_name)) +
        ...
        set.seed(20240410)
        g <- igraph::graph_from_graphnel(input_module)

        V(g)$gene_name <-  protein_complexes %>% 
        ...
        V(g)$mean_log_fc <- protein_complexes %>% 
          pull(mean_log_fc, name = gene_id)
         
        V(g)$protein_complex_id <- protein_complexes %>% 
          pull(protein_complex_id, name = gene_id)
        
        V(g)$in_ppic <- protein_complexes %>% 
          pull(in_ppic, name = gene_id)
        
        V(g)$temporal_expn_changes <- protein_complexes %>% 
          pull(temporal_expression_changes, name = gene_id)
     ...
     # plot using tidygraph
      ...
      pt_size <- ifelse(length(sel_nodes) <= 50, 18, 10)
      if ( length(unique(protein_complexes$temporal_expression_changes)) == 1 ){
        pt_legend <- list(size = c(4), alpha = c(1), shape = 21)
      }else{
        pt_legend <- list(size = c(2, 5), alpha = c(0.5, 1), shape = 21) 
      }
      
        
      module_graph <- ggraph(graph_tbl, 
             layout = 'stress') +
        geom_edge_arc(aes(width = weight),
                      strength = 0.2,
                      alpha = 0.1) + 
        geom_node_point(aes(fill = mean_log_fc,
                            color = protein_complex_id, 
                            shape = in_ppic, 
                            size = temporal_expn_changes,
                            alpha = temporal_expn_changes),
                        stroke = 2.5,
                        position = position_jitter(),
                        ) +
        geom_node_label(aes(label = gene_name),
                       repel = TRUE) +
        scale_fill_gradientn(colours = viridis::plasma(299),
                             limits = c(-10, 10),
                             oob = scales::squish, 
        ...
        scale_shape_manual(values = c("yes"=22, "no"=21), guide = NULL) +
        scale_alpha_manual(values = c("yes" = 1, "no" = 0.5), guide = NULL) +
        scale_size_manual(values = c("yes" = pt_size, "no" = floor(pt_size/2))) +
       
        guides(color = guide_legend(override.aes = list(size = 2)),
               size = guide_legend(override.aes = pt_legend)) + 
        labs(title = title) +
        theme_void()
      
        
        # use patchwork to create multi-panel plot
        plot_design <- "
        AAAAAAAABCD
        "
        ...
        figs <- module_graph +
                ...
                patchwork::plot_layout(guides = "collect", 
                                        design = plot_design) & theme(legend.position = 'left', plot.margin = margin(l = 0.25, r = 1.25, unit = "cm"))
    
        res <- list("module_graph" = figs,
                    "protein_complex_anno" = protein_complexes)
...
NOTE: only 20 of 124 genes from Profile 5 are represented in the PPI network!
- so thats why it is so underrepresented. 

```{r}
timecourse_profiles <- time_course %>% 
  select(combined_gene_clusters) %>% 
  distinct() %>% 
  pull(combined_gene_clusters, name = combined_gene_clusters)
...
# using all DEGs
enriched_ppi_modules <- purrr::map(timecourse_profiles, function(profile){
  print(profile)
...
# determine how many genes/proteins in the subnet are part of the time-course cluster
sel_modules <- lapply(enriched_ppi_modules, function(x) x[["module"]])
...
# FALSE       26       24       44       24       44
# TRUE        23       56      101       43        1
```


```{r}
# using up or down DEGs for the different profiles 
up_down_ppi_modules <- purrr::map(timecourse_profiles, function(profile){
  print(profile)
  if(grepl("1$|2$", profile)){
    print("Using down-regulated genes")
    DEGs_df <- filter(DEGs_df, num_upreg < 2)
  }else{
    print("Using upregulated-regulated genes")
    DEGs_df <- filter(DEGs_df, num_upreg >= 2)
  }
  
  pct <- ifelse(grepl("3$", profile), 0.05, 0.1)
  ppi_modules(time_course_res = time_course, 
              cluster_name = profile,
              msp_fit = fit_d456,
              input_network = pf_ppi_graph, 
...
```{r}
# checkout model fit
lapply(up_down_ppi_modules, function(x) plot(x[["model"]]))

# checkout the FDR thresholds on the scores
fdr_thresholds <- sapply(timecourse_profiles, function(x){
  pct <- ifelse(grepl("3$", x), 0.05, 0.1)
  pval <- quantile(up_down_ppi_modules[[x]]$pvalues, probs = pct)
  fdrThreshold(fdr = pval, fb = up_down_ppi_modules[[x]]$model)
})
...
all_genes_in_graphs <- sapply(up_down_ppi_modules, function(x) nodes(x$module)) %>% 
  unlist()

# generate color codes for the protein complexes graph figure
pc_annots_subset <- protein_complex_annots %>% 
  filter(gene_id %in% all_genes_in_graphs) %>%
  arrange(protein_complex_id) %>% 
  group_by(protein_complex_id) %>% 
  mutate(node_color = rep(colors_vector, 3)[dplyr::cur_group_id()]) %>% 
  ungroup() %>% 
  select(1:3, node_color, everything())


module_figs <- purrr::map(names(up_down_ppi_modules) %>% set_names(., value = .), function(x){
      print(x)
  
      profile <- x
      fig <- ppi_network_figures(input_module = up_down_ppi_modules[[profile]]$module, 
                          protein_complex_annots = pc_annots_subset,
                          DEGs = DEGs_df, 
                          title = profile)
      # 
      pdf(glue("figures/combat_seq/networks_ppi/time_course_{profile}_sel_module_genes_network_fcs_07.30.2024.pdf"), height = 11, width = 16)
      print(fig$module_graph)
      dev.off()

      return(fig)
})

module_figs$Profile1$module_graph
# module_figs$Profile2$module_graph
```

```{r}
graph_tables <- purrr::map_dfr(names(up_down_ppi_modules) %>% set_names(., value = .), function(x){
  
  profile <- x
  print(profile)
  
  scores <- data.frame(scores = up_down_ppi_modules[[profile]]$scores,
                     gene_id = names(up_down_ppi_modules[[profile]]$scores))
  
  pvals <- data.frame(pvalues = up_down_ppi_modules[[profile]]$pvalues, 
                    gene_id = names(up_down_ppi_modules[[profile]]$pvalues))

  tab <- data.frame(edges = names(up_down_ppi_modules[[profile]]$module@edgeData@data)) %>% 
    separate(edges, into = c("from","to"), sep="\\|", remove = FALSE)  %>% 
    pivot_longer(cols = c(from, to),
                 names_to = "ppi",
                 values_to = "gene_id") %>% 
    mutate(time_course_cluster = profile) %>% 
    left_join(., module_figs[[profile]]$protein_complex_anno %>% 
                    select(gene_id,
                           mean_log_fc, 
                           temporal_expression_changes,
                           hillier_protein_complex_id,
                           gene_name, description, 
                           corum_complex = corum_complex_clean ,
                           go_processes), 
              by = "gene_id") %>% 
    left_join(., scores, by = "gene_id") %>% 
    left_join(., pvals, by = "gene_id")
  
}) %>% 
  pivot_wider(id_cols = c(time_course_cluster,edges),
              names_from = ppi,
              values_from = c(gene_id, mean_log_fc:pvalues), 
              names_glue = "{ppi}_{.value}")  %>% 
  rename_at(vars(matches("gene_id")), ~gsub("_gene_id", "", .)) %>% 
  select(time_course_cluster, edges, from, to, everything()) 

dim(graph_tables)
head(graph_tables)
# graph_table %>% View()

# write.csv(graph_tables, "results/time_course/ppi/protein_protein_interaction_network_graphs.csv", row.names = FALSE)
```


```{r}
graph_tables %>% 
  filter(time_course_cluster == "Profile5",
         # from_temporal_expression_changes == "yes"
         ) %>% 
  View()
```
...
# examine different FDR cut-offs
p1_fdrs <- scanFDR(fb=up_down_ppi_modules$Profile1$model,
        fdr=c(0.001, fdr_thresholds[,1])) %>% 
  mutate(pvals = up_down_ppi_modules$Profile1$model$pvalues) %>% 
  rownames_to_column("gene_id") %>% 
  filter(gene_id %in% nodes(up_down_ppi_modules$Profile1$module))

# head(p1_fdrs)
# tail(p1_fdrs)
# summary(up_down_ppi_modules$Profile1$model)
# fdrThreshold(fdr = 0.001, fb = up_down_ppi_modules$Profile1$model)

sc <- sapply(up_down_ppi_modules$Profile1$module@nodeData@data, function(x) x$score)
# hist(sc,breaks = seq(-10, 64, by = 0.5))
# quantile(sc)

up_down_ppi_modules$Profile1$module
p1_filt <- largestScoreComp(network = up_down_ppi_modules$Profile1$module,
                 score = sc, 
                 level = 0)

# p1_filt
# plot(p1_filt)
p1_filt_fig <- ppi_network_figures(input_module = p1_filt, 
                          protein_complex_annots = protein_complex_annots,
                          DEGs = DEGs_df, 
                          title = profile)

# p1_filt_fig$module_graph
# quantile(sapply(p1_filt@nodeData@data, function(x) x$score))
```
...
```{r}
p1_genes <- time_course %>% 
  filter(combined_gene_clusters == "Profile1") %>% 
  pull(gene_id)

p1_eset <- cb_se[p1_genes, grep("Day[456]", cb_se$time_point)]
#Warning: No assay named 'exprs' found, renaming cpm to 'exprs'.
p1_eset <- as(p1_eset, "ExpressionSet")
exprs(p1_eset) <- log2(exprs(p1_eset) + 1)

dim(p1_eset)
# table(rowSums(exprs(p1_eset)) == 0)
length(p1_genes)
```
one called R with the PCCs and another called P with
the corresponding two-sided P-values for the null hypothesis of zero correlation. 

```{r}
# Warning: NaNs produced
pcc.estimates <- qpPCC(p1_eset)
pcc.estimates$P[1:5,1:5]
pcc.estimates$R[1:5,1:5]
```

```{r} 
tfs <- pf_tf_list %>% 
  filter(gene_id %in% intersect(gene_id, p1_genes))

dim(tfs)

p1_tfs <- unique(tfs$gene_id)
p1_tgs <- setdiff(p1_genes, p1_tfs)
length(p1_tfs) # 12
length(p1_tgs) # 321
```
...
```{r}
set.seed(20240417)
avgnrr.estimates <- qpAvgNrr(p1_eset,
                             nTests=1000, 
                             alpha=0.05,
                             pairup.i=p1_tfs,
                             pairup.j=p1_tgs,
...
```{r}
input %>% 
  filter(grepl("1644.1|0502.2",motif)) %>% 
  filter(!is.na(plot_fill))
...
```{r}
ortho_motif_PWMs$MA0502.2@profileMatrix

ortho_motif_PWMs$MA0502.2
```
```{r}
ortho_motif_PWMs$MA1644.1@profileMatrix
ortho_motif_PWMs$MA1644.1
```
