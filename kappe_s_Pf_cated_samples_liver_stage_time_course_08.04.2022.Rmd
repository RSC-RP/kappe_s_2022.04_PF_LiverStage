...
de_res <- dir("results/DEGs", pattern = "Day[456].+unfiltered_DEGs.csv", full.names = TRUE)
DEGs <- purrr::map(de_res, function(x){
  grp <- gsub(".+(Day[456].+)_vs.+","\\1", basename(x))
  
  df <- read.csv(x, comment.char = "#") %>% 
    janitor::clean_names() %>% 
    mutate(group = grp) %>% 
...
ppi_modules <- function(time_course_res, cluster_name, input_network, msp_fit, DEGs = NULL, percentile=0.1){
  ...
  if(!is.null(DEGs)){
      ...
      # remove NA values if present and order 
      in_pvalues <- in_pvalues[!is.na(in_pvalues)]
...
This genes was part of the selected PPI subnet/module identified by BioNET in Profile3 
PF3D7_1351600
- it is not found in any other time-course clusters (good)
- it IS found in the DEGs which is likely why it was selected 
- SO should it be included in the graph??
...
# lapply(enriched_ppi_modules, function(x) x[["module"]])
...
ppi_network_figures(input_module = enriched_ppi_modules$Profile1$module, 
...
```{r}
ppi_network_figures <- function(input_module, protein_complex_annots, gene_names_annot, DEGs){
    ...
    chk2 <- nrow(protein_complexes) == length(sel_nodes) # TRUE
    chk3 <- all(sel_nodes %in% protein_complexes$gene_id) # TRUE
    if(all(c(chk1, chk2, chk3))){
