...
# data(interactome) #Formal class 'graphNEL' [package "graph"] with 6 slots
# data(dataLym)
# BiocManager::install("EnrichmentBrowser", update = FALSE)
# BiocManager::install("DLBCL", update = FALSE)
...
GO_annots_TF <- GO_annots %>% 
  filter(grepl("transcription factor", go_term_name))
...
PPI <- openxlsx::read.xlsx("resources/PPI/1-s2.0-S221112471930909X-mmc6.xlsx") %>% 
  mutate(TF = ifelse(source %in% GO_annots_TF$gene_id, "TF", "Other"))
...
```{r}
PPI %>% 
  select(source, TF, sourceDescription, STRING,REACTOME) %>% 
  distinct() %>% 
  filter(TF == "TF")
```
...
length(unique(GRN$from)) #1512
# head(PPI)
# quantile(PPI$RFscore)
```
