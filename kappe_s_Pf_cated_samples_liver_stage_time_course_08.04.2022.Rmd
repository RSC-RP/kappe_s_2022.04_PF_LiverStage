...
```{r}
# rowData(day2_enr$enrichement_res)
# colData(day2_enr$enrichement_res)
# assays(day2_enr$enrichement_res)


df1 <- as.data.frame(assay(day2_enr$enrichement_res, "negLog10P")) %>% 
  rownames_to_column("motif")  %>% 
  pivot_longer(cols=2:3, 
               names_to="foldchange_bin",
               values_to="negLog10P")
  
df2 <- as.data.frame(assay(day2_enr$enrichement_res, "negLog10Padj")) %>% 
  rownames_to_column("motif")  %>% 
  pivot_longer(cols=2:3, 
               names_to="foldchange_bin",
               values_to="negLog10Padj")

df3 <- as.data.frame(assay(day2_enr$enrichement_res, "log2enr")) %>% 
    rownames_to_column("motif")  %>% 
    pivot_longer(cols=2:3, 
               names_to="foldchange_bin",
               values_to="log2enr")

day2_AP2_results <- df1 %>% 
  left_join(.,df2, by=c("motif","foldchange_bin")) %>% 
  left_join(., df3, by=c("motif","foldchange_bin"))
...
```{r}
gene_clusts_dfs$cluster1$betaTime %>%  quantile()
```
...
```{r}
time_course_df <- purrr::map_dfr(names(cluster_motifs), function(name){
      se <- cluster_motifs[[name]]
      
      df1 <- as.data.frame(assay(se$enrichement_res, "negLog10P")) %>% 
      rownames_to_column("motif")  %>% 
      pivot_longer(cols=2, 
                   names_to="time_course_coeff_bin",
                   values_to="negLog10P")
      
    df2 <- as.data.frame(assay(se$enrichement_res, "negLog10Padj")) %>% 
      rownames_to_column("motif")  %>% 
      pivot_longer(cols=2, 
                   names_to="time_course_coeff_bin",
                   values_to="negLog10Padj")
    
    df3 <- as.data.frame(assay(se$enrichement_res, "log2enr")) %>% 
        rownames_to_column("motif")  %>% 
        pivot_longer(cols=2, 
                   names_to="time_course_coeff_bin",
                   values_to="log2enr")
    ...
    results <- df1 %>% 
      left_join(.,df2, by=c("motif","time_course_coeff_bin")) %>% 
      left_join(., df3, by=c("motif","time_course_coeff_bin")) %>% 
      mutate(gene_cluster=name) %>% 
      select(gene_cluster, everything())
})

time_course_df
table(time_course_df$gene_cluster)

# write.csv(time_course_df,"motif_analysis/monaLisa/time_course_clusters_monaLisa_results.csv",row.names = FALSE)
```
...
Profile 4 Increasing over LS development (Cluster 5), 
Profile 5 constant in LS Development (Cluster 9), 
Profile 6 upregulated in mature LS forms (Cluster 7). 
...
time_course <- read.csv("time_course/masigpro/masigpro_significant_genes_degree2_d456_results.csv") %>% 
  ...
  mutate(combined_gene_clusters=case_when(
    ...
    grepl("5", gene_cluster) ~ "Profile4",
    grepl("9", gene_cluster) ~ "Profile5",
    grepl("7", gene_cluster) ~ "Profile6"
  )) %>% 
  dplyr::select(gene_id, matches("gene_cluster"), everything()) %>% 
  arrange(gene_cluster)
