...
all_PFMs <- readRDS("motif_analysis/JASPAR2022_PWMs.RDS")
...
motif_annots <- tags(all_PFMs) %>% 
  c(name(all_PFMs)) %>% 
  c(matrixClass(all_PFMs) %>%
      set_names(paste0(names(matrixClass(all_PFMs)), ".class"))) %>% 
  unlist() %>% 
  data.frame(values=.)  %>% 
  rownames_to_column("motif") %>% 
  arrange(motif) %>%
...
```{r}
sapply(all_PFMs[sel_motif_annots$motif], function(x) maxScore(Matrix(x))) %>% 
  data.frame(score=.) %>% 
  rownames_to_column("motif") %>% 
  arrange(score) %>% 
...
```{r}
summary_stats_TF <- expn_top_hits %>% 
  group_by(gene_cluster_factor, 
           analysis_factor_group,
           motif) %>% 
...
```{r}
create_motif_enr_plots(enr_df = input, 
...
create_motif_enr_plots <- function(enr_df, 
  ...
  num <- unique(summary_stats$motif)
  ...
  labs <- unique(enr_df$label) %>% rev()
  motif_enr <- ggplot(enr_df,
                      aes(x=motif, y=!!sym(factor_column), fill=plot_fill)) +
  ...
  top_logos <- purrr::map(top_motifs_per_clust, function(name){
    opts <- list(ID=name, matrixtype="PFM")
    pfm <- TFBSTools::getMatrixSet(JASPAR2020, opts) # can only use 2020 due to R version
    ggseqlogo(Matrix(pfm)) +
      labs(title=name) +
...
```{r}
top_motifs_per_clust[3]
```


```{r}
ggseqlogo(Matrix(PWMlist[[name]]))
```

```{r}
name_m <- top_motifs_per_clust[3]
opts <- list(ID=name_m, matrixtype="PFM")
pfm <- TFBSTools::getMatrixSet(JASPAR2020, opts)
pfm
# ggseqlogo(Matrix(pfm))
```

```{r}
opts <- list(ID="MA0502.2", matrixtype="PFM")
pfm <- TFBSTools::getMatrixSet(JASPAR2020, opts)
# pwm <- toPWM(pfm, type="prob")
# 
# # pwm
# # Matrix(pwm)[[1]] %>% colSums()
# ggseqlogo(pfms_dna$MA0018.2)
# ggseqlogo(Matrix(pfm))
ggseqlogo(Matrix(pwm))
```
...
ortholog_pf_ids <- orthoGroups %>% 
  ...
  ungroup() %>% 
  distinct() %>% 
  mutate(label=paste(motif_gene_name,gene_id,sep=":")) %>% 
...
```{r}
ortho_tfs_mat <- expression[ortholog_pf_ids$gene_id,rownames(input_d456)]
rownames(ortho_tfs_mat) <- unique(ortholog_pf_ids$label)


```

```{r}
# pdf("figures/combat_seq/ortholog_motif_heatmap_log2CPM_heatmap.pdf", height = 10, width = 10)
...
# pdf("figures/combat_seq/ortholog_motif_heatmap_row_zscores_heatmap.pdf", height = 10, width = 10)
pheatmap::pheatmap(ortho_tfs_mat,
                   annotation_col = select(design_input, analysis_factor_group),
