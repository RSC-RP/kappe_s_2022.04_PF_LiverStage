...
ppi_network_figures <- function(input_module, protein_complex_annots, DEGs, title = ""){
    ...
    protein_complexes <- protein_complexes %>% 
      mutate(in_ppic = ifelse(protein_complex_id != "other", "yes", "other"))
    ...
    if(all(c(chk2, chk3, chk4))){
     ...
     # plot using ggraph
      ...
      pt_size <- ifelse(length(sel_nodes) <= 50, 8, 5)
      ...
      module_graph <- ggraph(graph_tbl, 
        ...
        figs <- module_graph +
                ...
                patchwork::plot_layout(guides = "collect",
                                       ncol = 4, 
                                       width = unit(c(1, 2, 1, 1), c("null", "cm","null","null")))
                                # & theme(legend.position = 'left')
...
```{r}
sel_modules <- lapply(up_down_ppi_modules, function(x) x[["module"]])
...
```{r}
all_nodes <- sapply(sel_modules, nodes) %>% 
  unlist()

DEGs_df %>% 
  filter(gene_id %in% all_nodes) %>% 
  pull(mean_log_fc) %>% 
  quantile()
```
...
module_figs <- purrr::map(names(up_down_ppi_modules), function(x){
      print(x)
      profile <- x
      ppi_network_figures(input_module = up_down_ppi_modules[[profile]]$module, 
