     ...
     # plot using ggraph
      ...
      module_graph <- ggraph(graph_tbl, 
        ...
        scale_fill_gradientn(colours = viridis::plasma(299),
                             limits = c(-10, 10),
                             oob = scales::squish) +
                     # breaks = circlize::colorRamp2())
        scale_color_manual(values = pull(pcid, node_color, name = protein_complex_id)) +
        theme(legend.position = 'bottom') +
        theme_void()
        
        figs <- module_graph + fold_changes_heatmap + patchwork::plot_layout(guides = "collect")
          # list("fcs" = fold_changes_heatmap, 
          #             "network_graph" = module_graph)
        return(figs)
    } else {
