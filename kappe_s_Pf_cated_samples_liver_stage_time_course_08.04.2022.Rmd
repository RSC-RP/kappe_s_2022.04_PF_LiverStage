...
fit <- readRDS("time_course/masigpro_fit_p.vector_D2456.RDS")
tstep <- readRDS("time_course/masigpro_tstep_D2456.RDS")
...
```{r}
sample_order <- sigs_all$sig.genes$sig.profiles %>% 
  dplyr::select(Pf_Day0_Rep1:Pf_Day0_Rep4, 
                Pf_Day2_Rep1:Pf_Day2_Rep3, 
                Pf_Day4_Rep1,Pf_Day4_Rep3,Pf_Day4_Rep2, 
                matches("Pf_Day[56]"))  %>% 
  colnames()
...
ordered_genes <- gene_clusters_d456$cut[order(gene_clusters_d456$cut)]
...
sigs_all_df_d456 <- bind_cols(c(sigs_all_d456$sig.genes[c(2,4)], 
  ...
  dplyr::select(gene_cluster, everything())
...
sigs_all_df_d456 %>% 
  rownames_to_column("gene_id") %>% 
  dplyr::select(gene_id:betaTime2,matches("R|p.valor")) %>% 
  dplyr::filter(is.na(p.valor_Time)) %>% 
  arrange(desc(betaTime2))
```
...
```{r}
gene_clust_df <- ordered_genes %>% 
  as.data.frame() %>% 
  rename_all(~c("cluster")) %>% 
  mutate(cluster=as.factor(cluster))

row_ha <- rowAnnotation(df = gene_clust_df, 
                        col = list(cluster=ggpubr::get_palette("jco", 9) %>% 
                                     set_names(levels(gene_clust_df$cluster))))

expn_mat <- sigs_all_d456$sig.genes$sig.profiles[names(ordered_genes),]
...
```{r fig.height=10, fig.width=7}
# pdf("figures/combat_seq/masigpro/kappe_s_maSigPro_time_course_re-order_heatmap_d456.pdf",
    # height = 10, width = 10)
...
dir.create("homer/day2/up",recursive = T,showWarnings = FALSE)
dir.create("homer/day2/down",recursive = T, showWarnings = FALSE)


Day2 %>%
  dplyr::filter(logFC > 0) %>%
  write.table(.,"homer/day2/up/Day2_Heps_Pf_vs_sporozorite_combat-seq_corrected_upregulated.csv",
              sep="\t", quote=FALSE, row.names = FALSE)

Day2 %>%
  dplyr::filter(logFC < 0) %>%
  write.table(.,"homer/day2/down/Day2_Heps_Pf_vs_sporozorite_combat-seq_corrected_downregulated.csv",
              sep="\t", quote=FALSE, row.names = FALSE)
...
gene_clusts_dfs <- purrr::map(unique(sigs_all_df_d456$gene_cluster), function(clust){
  out <- here::here(file.path("homer",paste0("cluster",clust)))
...
do
  # NOTE: when trying to submit to HPC using qsub I was getting an error due not finding the Homer depencecies or something. So just ran locally. 
...
```{r}
# lapply(AP2_PWMlist,view_motifs, use.type='PWM')
```
...
```{r}
motif_enrichment_workflow <- function(targets_df, proms_gr, genome_bs, 
                                      region_length=1000,
                                      col_to_bin="logFC"){
...
save_monaL_plots <- function(monalisa_res, prefix){
    ...
    maxEnr <- ceiling(max(assay(monalisa_res$enrichement_res, "log2enr"), na.rm=TRUE))
    maxSig <- ceiling(max(assay(monalisa_res$enrichement_res,"negLog10P"), na.rm = TRUE))
    
    pdf(paste0(prefix,"_log2Enr_p.value_heatmap.pdf"), height = 10, width = 14)
    plotMotifHeatmaps(x = monalisa_res$enrichement_res, 
...
```{r}
cluster_motifs <- lapply(gene_clusts_dfs, 
                         motif_enrichment_workflow,
                         col_to_bin = "betaTime",
                         proms_gr = pf_proms_gr, 
                         genome_bs = genome)

names(cluster_motifs) <- names(gene_clusts_dfs)
```

```{r}
# cluster_motifs$cluster1$input_bins
# assay(cluster_motifs$cluster1$enrichement_res, "negLog10Padj")
```

```{r}
tcp <- lapply(names(cluster_motifs),function(x) save_monaL_plots(cluster_motifs[[x]],prefix = x))
```
...
```{r}
GO_annots <- read.delim("expression_data/PF3D7_GO_Terms_PlasmoDB_v58.txt", sep="\t") %>% 
  janitor::clean_names()
...
```{r}
clusterProfiler_bubble_plots <- function(df){
  
    input <- df %>% 
      mutate(neg_log10_FDR=-log10(p.adjust)) %>% 
      arrange(p.adjust)
    ...
    bubble_plot <- ggplot(temp, aes(x=neg_log10_FDR, y=ID)) +
    ...
    # return(bubble_plot)
    tab <-  ggplot(temp, aes(x = 0, y = ID, label=go_term_name, color=ontology)) +
        geom_text(size = 3,
                  hjust = 0,
                  nudge_x = 0)  +
        scale_y_discrete(expand = c(0.05,0)) +
...
day2_go_gsea <- GSEA(gene = d2_gsea_input, 
                    TERM2GENE = dplyr::select(GO_annots,ID=go_id, gene=gene_id))

day2_go_clean <- day2_go_gsea@result %>% 
  left_join(., GO_descriptions, by=c("ID"="go_id")) %>% 
  dplyr::select(ID,ontology:go_term_name, everything())
...
```{r}
# pdf("figures/combat_seq/bubble_plots/day2_vs_spz_ClusterProfiler_AllOntology_bubblePlot.pdf")
clusterProfiler_bubble_plots(day2_go_clean) 
# dev.off()
```
...
cluster_go <- purrr::map(names(gene_clusts_dfs), function(clust){
  ...
  gsea_input <- gene_clusts_dfs[[clust]] %>% 
    ...
    # pull(R.squared, name=gene_id)
  
  if (all(gsea_input < 0) ){
    type <- "neg"
  }else if ( all(gsea_input > 0 )){
    type <- "pos"
  }else{
    type <- "std"
  }
  ...
  go_enr_df <- go_enr@result %>%
    left_join(., GO_descriptions, by=c("ID"="go_id")) %>%
    dplyr::select(ID,ontology:go_term_name,setSize=Count,
           everything())

  res <- list("go_gsea"=go_enr, "go_df"=go_enr_df)
  return(res)
})
...
```{r}
sapply(cluster_go, function(x) dim(x$go_df))
# sapply(cluster_go, function(x) table(x$go_df$p.adjust < 0.05))
```

```{r}
cluster_go_dfs <- purrr::map_dfr(names(cluster_go), function(x) cluster_go[[x]]$go_df %>% 
                                   mutate(gene_cluster=x) %>% 
                                   dplyr::select(gene_cluster, everything()))

head(cluster_go_dfs)
# table(cluster_go_dfs$gene_cluster)
...
cluster_go_dfs %>% 
  ...
  mutate(FDR_sig_result=p.adjust < 0.05) %>% 
...
```{r}
purrr::map(names(cluster_go_gsea), function(clust){
  df <- cluster_go_gsea[[clust]]$go_df %>% 
          mutate_at(vars(ontology), ~case_when(
                    .=="N/A" ~ "unknown",
                    TRUE ~ .))
  ...
  for (ont in unique(df$ontology)){
    subset <- df %>% 
      filter(ontology == ont)
    pdf(paste0("figures/combat_seq/bubble_plots/",clust,"_ClusterProfiler_",ont,"_bubblePlot.pdf"),
        height = 5, width = 8)
    print(clusterProfiler_bubble_plots(subset))
    dev.off()
  }
...
top_paths <- cluster_go_dfs %>% 
  ...
  arrange(desc(neg_log10_FDR), .by_group = TRUE) %>% 
  dplyr::slice(1:10) %>% 
  ungroup() %>% 
...
svg("figures/combat_seq/bubble_plots/time_course_facet_plot_allOntology.svg", width = 15, height = 7)
bubble_plot +
  facet_wrap(~gene_cluster, scales = 'free', ncol = 5)
dev.off()
...
purrr::map(names(res),function(df_ID){ 
  svg(paste0("kappe_s_",df_ID,"_bubbleplot.svg"), height = 5, width = 10)
...
rsync -av --exclude degree1 time_course/masigpro $destination
rsync -av figures $destination
rsync -av GO_enrichment $destination
rsync -av motif_analysis --exclude results_jaspar2020_db --exclude outfiles $destination
```
