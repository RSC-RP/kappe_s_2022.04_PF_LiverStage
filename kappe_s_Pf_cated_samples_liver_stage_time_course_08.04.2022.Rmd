...
```{r}
# See https://doi.org/10.1371/journal.ppat.1001165.s016 for table used to map old IDs to new IDs on Plasmo DB
ap2_ids <- 
  # read.csv("ap2_old_ids_updated_PF3D7.csv") %>% 
  read.csv("expression_data/gene_annots/AP2_old_IDs_updated_PF3D7_IDs.csv") %>%
  janitor::clean_names()
...
```{r}
# dir.create("results/time_course")
...
# write.csv(counts, "results/time_course/trendCatcher_counts.csv", row.names = FALSE)
```
...
tc_res <- run_TrendCatcher(count.table.path = "results/time_course/trendCatcher_counts.csv",
                              baseline.t = 0,
                              time.unit = "day",
                              min.low.count = 1,
                              para.core.n = NA,
                              ...
                              show.verbose = FALSE)
# 
# saveRDS(tc_res, "results/time_course/trendCatcher_day0456_results.RDS")
```
...
# tf_dfs <- dir("references/AP2_motifs", pattern = "^P|^M", full.names = TRUE)
# tf_dfs
