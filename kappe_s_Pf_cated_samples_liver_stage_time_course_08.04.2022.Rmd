...
```{r}
table(input$gene_cluster,input$negLog10P >= 1.2)
```
...
input_withOrders <- input %>%
  mutate(motif=factor(motif, 
                      levels=rev(order_motifs_profile)),
...
```{r}
expn_top_hits %>% 
  filter(is.na(gene_cluster_factor))
```
...
create_motif_enr_plots <- function(enr_df, 
  ...
  motif_colors <- set_names(RColorBrewer::brewer.pal(n=num, name = "Set3"),
                              unique(top_motifs_per_clust))
  ...
  clust_labs <- enr_df %>% pull(label, name="gene_cluster_factor")
  incl_hits <- enr_df %>% 
  ...
  top_logos <- purrr::map(top_motifs_per_clust, function(name){
    ggseqlogo(PWMlist[[name]]@profileMatrix) +
      labs(title=name) +
  ...
  return(time_ribbon=time_ribbon, time_cluster=time_cluster,motif_enr=motif_enr, top_logos=top_logos)
}
```
