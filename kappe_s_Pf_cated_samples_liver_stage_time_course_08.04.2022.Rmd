...
orthoMCL <- read.delim("motif_analysis/homer/groups_OrthoMCL-6.14.txt", 
  ...
  separate(V1, into=c("ortholog_group","accession"), 
           sep=": ", remove=FALSE) %>% 
  select(ortholog_group,accession,full_result=V1)
...
```{r}
library(UniProt.ws, lib.loc = "/gpfs/home/jsmi26/R/x86_64-pc-linux-gnu-library/4.1")
```
...
orthoGroups_species %>% 
  janitor::clean_names() %>% 
  # filter(grepl("Homo sapiens", taxon_name)) %>% 
  filter(!grepl("-old", accession)) %>% 
  mutate(taxon_clean=gsub("^([A-Z][a-z]+\\s[a-z]+)\\s.+", "\\1", taxon_name)) %>% 
  dplyr::select(accession, taxon_name, taxon_clean, 
         everything())
...
```{r}
availableUniprotSpecies(pattern="Aspergillus aculeatus")
options(url.method="libcurl")
asp_res <- UniProt.ws(taxId=9606)
```
...
BiocManager::install("UniProt.ws", force = TRUE, 
                     lib="/gpfs/home/jsmi26/R/x86_64-pc-linux-gnu-library/4.1")
```
...
```{r}
sel_tc_hits <- profile_all_bins %>% 
  
  mutate(gene_cluster_factor=as.factor(gene_cluster)) %>% 
  mutate(p_value=1/10^negLog10P,
         adj.p_value=1/10^negLog10Padj, 
         is_significant=ifelse(negLog10P > 1.3, "p < 0.05", "NS"), 
         # label = paste0("cluster",gene_cluster),
         label = gene_cluster,
  ...
  mutate(plot_fill=case_when(
    ...
    gene_cluster == "Profile2" & negLog10P > 1.09 ~ log2enr,
    TRUE ~ NA_real_)) %>% 
  ungroup() %>% 
...
```{r}
order_motifs_profile <- tail(c(clust_motifs$labels[clust_motifs$order],"PF11_0442"), n=22) %>% 
  unique()
...
ap2_expn_top_hits <- purrr::map(top_motifs_per_clust_df$gene_cluster,function(x){
...
```{r}
input <- ap2_expn_top_hits$`cluster1:PF10_0075_D2` %>% 
  group_by(analysis_factor_group) %>% 
...
ggplot(input,
       aes(x=analysis_factor_group, y=log2_CPM)) +
  geom_ribbon(aes(ymin=ymin, ymax=ymax, group=input_id),
              fill = "gold", alpha = 0.075) +
  geom_boxplot(aes(x=analysis_factor_group,
                   y=log2_CPM,
...
# ggplot(input,
#          # mutate(analysis_factor_group=factor(analysis_factor_group, levels=rev(levels(analysis_factor_group)))),
#        aes(y=analysis_factor_group, x=log2_CPM)) +
#   ggridges::geom_density_ridges(aes(fill=analysis_factor_group)) +
#   scale_fill_viridis_d(option="D", begin=0.2, end=0.9) +
#   theme_classic()
...
# top_logos$`cluster1:PF10_0075_D2`
```
...
# pdf("figures/motif_analysis/time_point_profile_ap2_heatmap_seqlogos_tileplot_rows.pdf", height = 12, width=15)
...
table(sel_profile_hits$gene_cluster, sel_profile_hits$is_significant)
```
