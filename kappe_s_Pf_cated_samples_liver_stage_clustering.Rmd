...
```{r}
counts_matrix_cated <- read.csv("raw_counts/kappe_s_PfHsMmu_raw_counts.csv") %>% 
  select(all_of(c("gene_name", sample_manifest_cated$sample_id_cat))) %>% 
...
mapping_stats <- read.csv("raw_counts/kappe_s_PfHsMmu_counts_stats.csv")  %>% 
  select(all_of(c("gene_name", sample_manifest_cated$sample_id_cat))) 
...
dim(TMMCPM_all)  #26,459    27
head(TMMCPM_all[,1:5])

# write.csv(rownames_to_column(TMMCPM_all,"gene_name"), "expression_data/kappe_s_PfHsMmu_cated_samples_TMM_CPM.csv", row.names = FALSE)
```
...
table(species_genes$genome)
    ...
    #  50914      35582       5645 
species_genes %>% 
  filter(gene_name %in% rownames(TMMCPM_all)) %>% 
  group_by(genome) %>% 
  dplyr::count()
```

So there is only 19% coverage of the human genes after filtering to remove low abundance genes -- ughhh 
There is ~33% coverage of the mouse genes, and 85% coverage of the Pf genes :/

```{r}
pf_genes <- species_genes %>% 
  filter(genome=="Plasmodium")

pf_counts <- counts_matrix_cated[pf_genes$gene_name, ]
...
dim(pf_TMMCPM)  #4980   27
head(pf_TMMCPM[,1:5])

# quantile(as.matrix(pf_TMMCPM))
# quantile(as.matrix(TMMCPM_all))

# cbind(pf_dge$samples$norm.factors, #these norm factors WAYY inflated tbh
#       dge_all$samples$norm.factors)

# write.csv(pf_TMMCPM,"expression_data/kappe_s_PfGenomeOnly_cated_samples_TMM_CPM.csv")
```
...
# sums_old[order(sums_old)]
# sums_mine[grep("L2Pf6", names(sums_mine))]
```
...
sum(dge_all$counts[,"Day2.GFP_L1Pf2xTR2_L1Pf2_S1"]) #18,224,130
sum(pf_dge$counts[,"Day2.GFP_L1Pf2xTR2_L1Pf2_S1"]) #15,612
```
...
# write.csv(mapping_stats_comparison, "expression_data/kappe_s_mapping_stats_cated_samples_06.16.2022.csv",
#           row.names = FALSE)
```
...
grouped_plots <- purrr::map(groups, function(grp){
  ...
  fname <- file.path("figures/sample_qc/concatenated_samples",paste0("kappe_s_cated_samples_all_genomes_",grp,".pdf"))
  print(fname)
...
pf_samples <- unique(pf_expression$CPM_anno$sample_id_cat)

pf_density_plots <- purrr::map(pf_samples,
...
```{r fig.height=6}
# pdf("figures/exploratory/kappe_s_Pf_LS_cated_samples_labels_500_mostVarGenes_pfGenomeOnly.pdf", height = 6, width = 10)
pf_PCA$pca_plots[[1]] +
...
# pdf("figures/exploratory/kappe_s_Pf_LS_cated_samples_500_mostVarGenes_pfGenomeOnly.pdf", height = 6, width = 16)
...
```{r}
# dge_pf_pos <- edgeR::DGEList(counts=pf_expression$counts[, pf_samples$sample_id_cat] %>% 
...
# col <- colorRampPalette(c("cyan1", "cyan2", "cyan3", "cyan4","azure4","magenta4", "magenta3", "magenta2", "magenta1"))(n=299)
col <- colorRamp2(c(-3.5,-1.5, 0,1.5, 3.5), c("dodgerblue4", "dodgerblue1","white","red1", "red4"), space="sRGB") #use for breaks.

# pdf("figures/parasites_clustering/kappe_s_pf_pos_top500MostVarGenes_allSamps_06.16.2022.pdf", height = 8, width = 14)
...
# pdf(paste0("figures/parasites_clustering/kappe_s_pf_cated_samples_Pos_largestFC_",type,"_06.16.2022.pdf"), height = 8, width = 14)
...
create_pf_heatmaps(TMMCPM=pf_expression$TMMCPM[,pf_samples$sample_id_cat] %>% 
                   ...
                   genelist=unique(day456_sel$gene_name),
                   heatmap_colors = col)
# dev.off()
```
...
purrr::map(paste0("Day",c(2,4:6)), function(day){
  ...
  # pdf(paste0("figures/parasites_clustering/kappe_s_pf_cated_samples_pos_largestFC_",day,"_06.16.2022.pdf"),
  #     height = 8, width = 14)
