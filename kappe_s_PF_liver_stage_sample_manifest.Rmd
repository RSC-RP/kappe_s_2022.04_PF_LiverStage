...
library(groundhog)
pkgs <- c("stringr","magrittr","ggplot2", "gridExtra","dplyr","tidyr","tibble")
groundhog.library(pkgs, "2022-04-14")
getwd()
```
...
First, the fastq files must be re-organized and then map filenames to the sample sheets.  

  #   grepl("210408",symlinks) ~ paste0("/active/kappe_s/kappe/Gigliola/NextSeq_Runs/210408_NS500259_0368_AH33V3BGXH/demultiplexed-Data",filename),
  #   grepl("201014", symlinks) ~ paste0("/active/kappe_s/kappe/Gigliola/NextSeq_Runs/201014_NB501859_0214_AHY3V5BGXC/Data/Intensities/BaseCalls", filename)
...
```{r eval=FALSE}
#Only run this once to organize data for the rnaseq_counts workflow
data_path <- "resources/fastq"

mksymlinks <- lapply(unique(fqs$sample_id),function(Sample_id){
  
  subset <- fqs %>% 
    filter(sample_id==Sample_id)
  analysis_dir <- file.path(data_path,Sample_id)
  dir.create(analysis_dir, recursive = TRUE)

  for(i in 1:2){
    target <- file.path(analysis_dir, subset[i,"analysis_filename"])
    command <- paste("ln -svf",subset[i,"fixed_fastq_paths"], target)
    system(command)
  }
})
```
...
fqs_clean <- fqs %>% 
  mutate(symlink_fastq_paths=dir(data_path, recursive = T, full.names = TRUE)) %>% 
