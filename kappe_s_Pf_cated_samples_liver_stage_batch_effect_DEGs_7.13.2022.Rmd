...
species_genes %>% 
  mutate(gene_order=factor(gene_id, levels=rownames(combat_de$Day2_Heps_Pf$eBayesFit$coefficients))) %>% 
  select(gene_name, gene_id, gene_order) %>% 
...
```{r}
pal <- circlize::colorRamp2(c(-10,-6,-2, 0,2, 6, 10),
                      c("deepskyblue4", "deepskyblue2", "deepskyblue","white", "red", "red2", "red4"),
                      space="LAB")
...
```{r}
gene_annots <- read.csv("/active/taylor_s/people/ybae/CP-Bioinformed/kappe_s_2022.09_PF_LiverStage/PfHsMmu_GRCh38_GRCm39_Pf3D7v58_genes_annots.csv")
gene_annots_pf <- gene_annots[grepl("^PF", gene_annots$gene_id),] # 5720
```

```{r}
all_Genes <- all_Genes %>% mutate(annotated=case_when(
  gene_id %in% gene_annots_pf$gene_id ~ "annotated",
  TRUE ~ "na"
))
all_Genes
