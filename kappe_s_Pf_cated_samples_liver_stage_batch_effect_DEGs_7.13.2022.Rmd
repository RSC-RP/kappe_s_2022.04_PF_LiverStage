...
dim(pf_genes)
table(pf_genes$gene_biotype)
```

```{r}
pf_genes %>% 
  filter(is.na(gene_biotype))
  # filter(gene_id == 'PF3D7_0935400' | gene_id == 'PF3D7_0935390')
```
...
```{r eval=FALSE}
# This is for the Time-course analysis/PPI network graph analysis to have p-value vectors for all genes at all time-points
grps <- levels(pf_combat_dge$samples$analysis_factor_group)[-1]
ref <- levels(pf_combat_dge$samples$analysis_factor_group)[1]

combat_de_nofilter <- purrr::map(grps, function(grp){
  comparison <- c(ref, grp)
  subset <- pf_combat_dge[,pf_combat_dge$samples$analysis_factor_group %in% comparison]
  subset$samples$analysis_factor_group <- droplevels(subset$samples$analysis_factor_group)
...
names(combat_de_nofilter) <- grps
# saveRDS(combat_de_nofilter,"results/DEGs/kappe_sHeps_Pf_vs_sporozorite_combat-seq_corrected_DEGs_nogenefilter_list.RDS")
```   

```{r}
# combat_de_nofilter$Day2_Heps_Pf$all_genes
sapply(combat_de_nofilter, function(x) dim(x$all_genes))
lapply(combat_de_nofilter, function(x) head(x$all_genes))
```
...
--> Author of Barplot figure: Yeji Bae
