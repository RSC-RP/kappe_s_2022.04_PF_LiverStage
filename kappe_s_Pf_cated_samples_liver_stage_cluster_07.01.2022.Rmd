...
```{r}
full_manifest <- read.csv("samples/Kappe_s_2022.04_PF_LiverStage_sample_manifest_v2.csv")
# head(full_manifest)
```
...
```{r}
all_TPM <- read.csv("/active/taylor_s/people/jsmi26/CP-Bioinformed/kappe_s_2022.04_rnaseq_quant/normalized_counts/kappe_s_PfHsMmu_GRCh38_GRCm39_Pf3D7v58_TPM.csv") %>% 
  set_rownames(.$gene_name)
...
mapping_stats_comparison <- mapping_stats_long %>% 
  ...
  ungroup() %>% 
  left_join(., sample_manifest_cated, by="sample_id_cat") %>% 
...
```{r}
pf_seqdepth_per_sample <- pf_expression$counts_anno %>% 
  group_by(sample_id_cat,analysis_group) %>% 
  summarise(total_pf_pe_reads=sum(counts),
            .groups = "keep") %>% 
  ungroup() %>% 
  arrange(analysis_group) 

# pf_seqdepth_per_sample

pf_seqdepth <- pf_seqdepth_per_sample %>% 
  group_by(analysis_group) %>% 
  summarize(mean_pf_pe_reads=round(mean(total_pf_pe_reads))) %>% 
  ungroup()

pf_seqdepth
# write.csv(pf_seqdepth,"liver_stage_Pfalciparum_aligned_rnaseq_seqdepth_by_group.csv", row.names = FALSE)

# pf_seqdepth_per_sample %>% 
#   filter(analysis_group=="sporozorite") %>% 
#   arrange(desc(total_pf_pe_reads))
```

```{r}
all_cts <- read.csv("raw_counts/kappe_s_PfHsMmu_GRCh38_GRCm39_Pf3D7v58_raw_counts.csv")

# head(all_cts)
```

```{r}
all_cts_long <- all_cts %>% 
...
```{r}
coverage_by_genome <- all_cts_long %>% 
  group_by(sample_id_cat) %>% 
  mutate(total_counts=sum(counts)) %>% 
  group_by(analysis_group, 
           sample_id_cat, 
           genome,
           total_counts, .add = TRUE) %>% 
  summarise(total_per_genome=sum(counts), 
            .groups = 'keep') %>% 
            # mean=mean(counts), 
            # median=median(counts), 
            # sd=sd(counts)) %>% 
  ungroup() %>% 
  mutate(percent_of_total=round(total_per_genome/total_counts * 100, digits = 3))


# write.csv(coverage_by_genome,"qc_data/sequencing_coverage_by_sample_per_genome.csv", row.names = F)

ave_coverage_by_genome <-  coverage_by_genome  %>% 
  group_by(analysis_group, genome) %>% 
  summarise(mean_pct=round(mean(percent_of_total), digits=3))

# write.csv(ave_coverage_by_genome, "qc_data/average_percent_of_reads_aligned_per_genome_by_group.csv", row.names = F)

mean_total_cov <- coverage_by_genome %>% 
  group_by(analysis_group) %>% 
  summarise(mean_coverage=mean(total_counts))


ave_coverage_by_genome %>% 
  left_join(., mean_total_cov, by="analysis_group") %>% 
  filter(grepl("Pf\\+",analysis_group),
         grepl("GRChm39", genome)) %>% 
  mutate(mouse_reads=mean_coverage*(mean_pct/100))
...
pf_pos_sel <- full_manifest %>% 
  filter(grepl("Pf\\+|sporozorite", analysis_group))

# pf_pos_sel

to_reqseq <- all_cts[,c("gene_name",pf_pos_sel$sample_id)] %>% 
  filter(gene_name %in% pf_genes$gene_id) %>% 
  pivot_longer(cols=-gene_name,
               names_to="sample_id",
               values_to="counts") %>% 
  left_join(., pf_pos_sel, by="sample_id") %>% 
  group_by(sample_id, analysis_group) %>% 
  summarise(total_pf_pe_reads=sum(counts)) %>% 
  ungroup() %>% 
  arrange(analysis_group, desc(total_pf_pe_reads)) %>% 
  left_join(., select(full_manifest, sample_id, 
                      index_1:read2_fastq),
            by="sample_id")

to_reqseq
# write.csv(to_reqseq,"liver_stage_Pfalciparum_pos_samples_reqsequence.csv", row.names = FALSE)
```
