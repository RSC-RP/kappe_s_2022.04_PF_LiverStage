...
```{r}
seq_depth <- mapping_stats_comparison %>% 
  select(sample_id_cat, total_reads, analysis_group) %>% 
  distinct() %>% 
  group_by(analysis_group) %>% 
  summarise(mean_pe_reads=mean(total_reads)) %>% 
  ungroup() %>% 
  mutate(mean_pe_reads=formatC(mean_pe_reads, format = "e", digits = 1)) %>% 
  arrange(analysis_group)

seq_depth
# table(seq_depth$analysis_group)
# write.csv(seq_depth, "liver_stage_rnaseq_seqdepth_by_group.csv", row.names = FALSE)
```
