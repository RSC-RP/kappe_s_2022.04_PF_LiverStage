...
include <- intersect(ptex_ids, rownames(log2_TPM_expn))
...
ggplot(summary_ptex_expn, aes(x=time_point, y=median, group=gene_id)) +
  geom_line(aes(color=gene_id), alpha=0.8, 
            position = pd) +
  geom_point(aes(color=gene_id), alpha=0.8, 
             position = pd) + 
...
# non-log expression 
include_pexel <- intersect(pexel_genes_ided, rownames(TPM))
