...
# Biostrings::writeXStringSet(proteins_aa, "cvgs_pexel_domains/PF3D7_full_length_protein.fasta")
```
```{r}
...
# cvg_expn <- log2(assay(cb_se,"cpm")+1)[,sel_samps$sample_id_cat]
include <- base::intersect(cvgs$gene_id, rownames(cvg_expn))
cvg_expn <- cvg_expn[include,]
# table(rowSums(cvg_expn) > 0) #131  remove all zero count genes
...
purrr::map(sheet_names, function(cvg_type){
  ...
  # svglite(paste0("figures/uncorrected/cvgs_pexel/cvg_", cvg_type,"_TPM_expression_boxplot.svg"),
...
# quantile(as.matrix(cvg_expn[gene_order, ]), probs = seq(0,1,length.out=11))
heat_colors <- circlize::colorRamp2(breaks=c(0,1,3,6,9), colors = viridis::turbo(5))
mat <- as.matrix(cvg_expn[gene_order, ]) 
colnames(mat) <- sel_samps$analysis_group_id

# pdf("figures/uncorrected/cvgs_pexel/CGVs_timepoints_D456_log2TPM_heatmap.pdf", height = 12, width = 10)
Heatmap(matrix = mat, 
        col = heat_colors,
        name = "log2 TPM",
        split = gap_index,
...
# write.csv(pexel_hits_table, "cvgs_pexel_domains/pexel_motif_hits_150aa_summary_table.csv", row.names = F)
```
...
ggplot(ptex_expn_long, aes(x=analysis_factor_group, y=log2TPM, fill=gene_id)) +
  geom_boxplot(alpha=0.2, color="grey50") +
  geom_point(aes(color=analysis_factor_group),
             position = position_jitterdodge(jitter.width = 0.1),
             size=2, alpha=0.75) +
  scale_color_viridis_d(begin=0,end=0.8,direction = -1) +
  scale_fill_manual(values=ggpubr::get_palette("Blues", 5), 
                    labels=labels) +
  guides(fill = guide_legend(override.aes = list(size = 0, alpha=0.3) )) +
  labs(x="") +
  theme_classic() 
# dev.off()
```
...
canon_expn <- log2(pexel_expn[canon_gene_ids, ]+1)
# dim(canon_expn) #669

colanno <- samples_include %>% 
  select(time_point=analysis_factor_group) %>% 
  HeatmapAnnotation(df = ., which = "column",
                    col = list(time_point=ggpubr::get_palette("jco",5) %>%
                                          set_names(unique(pf_samples$analysis_factor_group))))
...
# heat_colors <- colorRampPalette(viridis::turbo(10))(299)
# quantile(as.matrix(canon_expn), probs = seq(0,1,length.out=11))
heat_colors <- circlize::colorRamp2(breaks=c(0,1,3,6,9,12), colors = viridis::turbo(6))
mat <- as.matrix(canon_expn[rowanno_df$gene_id, samples_include$sample_id_cat])
colnames(mat) <- samples_include$analysis_group_id
rownames(mat) <- rowanno_df$label

# pdf("figures/uncorrected/cvgs_pexel/PEXEL_canonical_timepoints_D456_log2TPM_heatmap.pdf", height = 20, width = 12)
Heatmap(matrix = mat, 
        col = heat_colors,
        name = "log2 TPM",
        top_annotation = colanno, 
...
head(known_exported)
table(known_exported$has_signal_pep) %>% View()
# FALSE  TRUE 
#   257   165 
...
```{r}
signalp_res <- read.delim("cvgs_pexel_domains/signalp6.0/prediction_results.txt", sep='\t', comment.char = "#", header = FALSE) %>% 
...
# write.csv(signalp_res,"cvgs_pexel_domains/signalp6.0/prediction_results_annotated.csv", row.names = FALSE)
```
...
```{r}
signalp_gtf <- rtracklayer::import("cvgs_pexel_domains/signalp6.0/output.gff3") %>% 
  as.data.frame() %>% 
...
known_exported %>% 
  ...
  dplyr::count() %>% 
  ungroup() %>% 
  mutate(source=ifelse(is.na(source), "plasmoDB", source)) %>% View()
...
# write.csv(signalp_pexel_results, "cvgs_pexel_domains/canonical_pexel_motif_hits_150aa_signalP_hits_100aa_expression_TPM.csv", row.names = FALSE)
```

### Non-canonical 

```{r}
# sapply(pexel_noncanon_1TPM_dfs, function(x) table(x[,grep("n_matches",colnames(x))]))


t <- sapply(pexel_noncanon_1TPM_dfs, function(x) select(x, gene_id, passed_threshold) %>%  distinct() %>%  pull(passed_threshold) %>%  table() ) 
colnames(t) <- names(pexel_noncanon_1TPM_dfs)
t
```
...
#   glue::glue("cvgs_pexel_domains/noncanonical_pexel_motif_{x}_hits_150aa_signalP_hits_100aa_expression_TPM.csv"), row.names = FALSE))
```
