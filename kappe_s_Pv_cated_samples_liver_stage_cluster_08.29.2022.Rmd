...
```{r}
mapping_stats <- read.csv("raw_counts/kappe_s_PvHsMmu_GRCh38_GRCm39_P01v58_counts_stats.csv")  %>% 
  rename_at(vars(gene_name), ~c("gene_id")) %>% 
  rename_all(~gsub("\\.", "", .)) %>% 
  select(all_of(c("gene_id", pvivax_manifest$sample_id))) 
...
```{r}
orthologs <- read.csv("expression_data/PVP01_PF3D7_GenesByOrthologs_Summary.csv") %>% 
  janitor::clean_names() %>% 
...
# write.csv(pv_TMMCPM,"expression_data/kappe_s_PVP01_genes_cated_samples_TMM_CPM.csv")
```
