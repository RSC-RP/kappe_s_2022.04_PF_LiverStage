...
FRG huHep infection, liver extraction and RNA extraction

All animal procedures were conducted in accordance with and approved by the Center for Infectious Disease Research Institutional Animal Care and Use Committee (IACUC). The Center for the Infectious Disease Research adheres to the NIH Office of Laboratory Animal Welfare standards (OLAW welfare assurance #A3640-01). FRG huHep mice were infected intravenously through the tail vein with sporozoites isolated from the salivary glands of infected mosquitoes. Sporozoites inocula were suspended in 100 µl of RPMI media. Mice were euthanized and livers were perfused with PBS through the hepatic portal vein, removed, and separated into lobes. Each lobe was cut into two sections and one was reserved for IFA and one was reserved for RNA extraction. Lobes used for RNA extraction were pooled and added to TRIzol reagent (Invitrogen). Pooled liver lobes were homogenized in Trizol reagent and RNA was extracted using the Rneasy purification kit (Qiagen).


RNA capture and sequencing library preparation

A SureSelect XT custom oligo library was designed with Agilent (Design ID: S0782852) to enrich for P. vivax specific cDNA among the pool of human, mouse and parasite cDNA obtained from the RNA extraction from the humanized mouse liver. 85,000 120 bp probes were tiled every 100bp across the entire P. vivax Sal I genome. Sequences >30% similar to human sequences were excluded. Sequencing libraries were prepared according to the SureSelect XT RNA Target Enrichment for Illumina Multiplexed Sequencing protocol from Agilent (Ref: 5190-4393). Libraries were analyzed using a BioAnalyzer and were quantified using qPCR.
...
```{r}
orthologs <- read.csv("resources/gene_annots/PVP01_PF3D7_GenesByOrthologs_Summary.csv") %>% 
  janitor::clean_names() %>% 
...
```{r}
pv_orthologs <- read.csv("resources/gene_annots/PF3D7_PVP01_GenesByOrthologs_Summary.csv") %>% 
    janitor::clean_names() %>% 
    filter(!duplicated(gene_id))
...
```{r}
all_TPM <- read.csv("expression_data/kappe_s_PvHsMmu_GRCh38_GRCm39_P01v58_TPM.csv") %>% 
  rename_all(~gsub("\\.","", .))
...
```{r}
# svglite::svglite("figures/uncorrected/pvivax_human_day8control_gene_expn_nofilter_densityPlot.svg",height = 5, width = 7, fix_text_size=FALSE, system_fonts = list(sans = "Open Sans"))
...
# svglite::svglite("figures/uncorrected/pvivax_human_day8control_gene_expn_filtered_densityPlot.svg", height = 5, width = 7, fix_text_size=FALSE, system_fonts = list(sans = "Open Sans"))

# pdf("figures/uncorrected/pvivax_human_day8control_gene_expn_filtered_densityPlot.pdf", height = 5, width = 7)
...
```{r}
pir_genes <- read.csv("references/Pvivax_PIR.csv") %>% 
  janitor::clean_names() %>% 
  group_by(product_description) %>% 
  mutate(PIR_products = case_when(
    grepl("PIR|VIR", product_description) ~ product_description, 
    TRUE ~ "other")) %>% 
  ungroup()

# head(pir_genes)
# dim(pir_genes) #1266   21
# any(duplicated(pir_genes$gene_id))
table(pir_genes$PIR_products)
```

```{r}
pv_manifest <-  pvivax_manifest %>% 
  filter(group=="Cont") %>% 
...
```{r}
pir_expn <- log2(pv_TPM_D8[,pv_D8]+1)
colnames(pir_expn) <- names(pv_D8)

include <- base::intersect(pir_genes$gene_id, rownames(pir_expn))
pir_expn <- pir_expn[include,]
pir_expn <- pir_expn[rowSums(pir_expn) > 0, ]

head(pir_expn)
# dim(pir_expn) #241   3
# any(duplicated(rownames(pir_expn)))
```

```{r}
pir_expn_long <- pir_expn %>% 
  as.data.frame() %>% 
  rownames_to_column("gene_id") %>% 
  pivot_longer(cols = -gene_id,
               names_to = "analysis_group_id",
               values_to = "log2TPM") %>% 
  inner_join(., pv_manifest, by="analysis_group_id") %>%
  left_join(., pir_genes, by=c("gene_id")) %>%
  select(gene_id:log2TPM, product_description, everything())

pir_expn_long
# table(pir_expn_long$type)
```

```{r}
pir_expn_summary_stats <- pir_expn_long  %>% 
...
# check on these without PIR/VIR in the list?
```

```{r fig.height=10, fig.width=10}
library(ComplexHeatmap)

colanno_df <- pv_manifest %>% 
  select(time_point = analysis_group_id) 

mat <- as.matrix(pir_expn[, rownames(colanno_df)]) 
mat <- mat[rowSums(mat) > 0, ]

colors <-  paste0("turquoise",2:4) %>% 
  # ggpubr::get_palette("jco",3) %>% 
  magrittr::set_names(., value = colanno_df$time_point)

colanno <-  HeatmapAnnotation(df = colanno_df,
                    which = "column",
                    height = unit(10,"mm"),
                    col = list("time_point" = colors))


rowanno <- pir_genes %>% 
  filter(gene_id %in% rownames(mat)) %>% 
  select(gene_id, PIR_products) %>% 
  column_to_rownames("gene_id") 

gap_index <- rowanno %>%
  group_by(PIR_products) %>%
  mutate(n=n()) %>%
  ungroup() %>%
  arrange(n) %>%
  mutate(PIR_products=factor(PIR_products, levels=unique(PIR_products))) %>%
  select(-n)

rowanno <- rowanno %>%
  mutate(PIR_products=factor(PIR_products, levels=unique(gap_index$PIR_products))) %>%
  arrange(PIR_products)

gene_order <- rownames(rowanno)
n <- length(unique(rowanno$PIR_products))
rowanno <- rowanno %>% 
  HeatmapAnnotation(df = . , which = "row",
                    width = unit(10,"mm"),
                    col = list("PIR_products"=ggpubr::get_palette(palette = "npg", n) %>%
                                        set_names(unique(rowanno$PIR_products))))
...
# svglite::svglite("figures/uncorrected/cvgs_pexel/Pvivax_PIR_genes_timepoints_D8_log2TPM_heatmap.svg",fix_text_size=FALSE, system_fonts = list(sans = "Open Sans"), height = 15, width = 15)
png("figures/uncorrected/cvgs_pexel/Pvivax_PIR_genes_timepoints_D8_log2TPM_heatmap.png", height = 15, width = 15, units = "in", res = 100)
Heatmap(matrix = mat,
        col = heat_colors,
        name = "log2 TPM",
        split = gap_index,
...
```{r}
# pf_TPM %>% 
#   filter(gene_name %in% c("PF3D7_0304600","PF3D7_0818900"))
```
...
# svglite("figures/uncorrected/results/pvivax_vs_pfalciparum_orthologs_histogram.svg", height = 5, width = 10, fix_text_size=FALSE, system_fonts = list(sans = "Open Sans"))
h1 + h2
# dev.off()
```
...
```{r}
pf_TPM_D6 %>% 
  filter(grepl("GDV1", gene_name) | grepl("antisense", description))
...
sum(sapply(list(both,pv_only,pf_only), length))
```

```{r}
grep("PF3D7_0935390",pv_exp)
grep("PF3D7_0935390",pf_exp)
```
...
# write.csv(pf_TPM_D6, "results/pvivax_vs_pfalciparum/pfalciparum_day6_orthologs_TPM.csv",row.names = FALSE)
...
# write.csv(pv_TPM_D8, "results/pvivax_vs_pfalciparum/pvivax_day8_orthologs_TPM.csv",row.names = FALSE)
```

```{r}
pf_TPM_D6 %>% 
  filter(grepl("PF3D7_0935390", gene_id))

pv_TPM_D8 %>% 
  filter(grepl("PF3D7_0935390", p_falciparum_ortholog))
```
...
```{r}
# svglite::svglite("figures/uncorrected/results/pvivax_vs_pfalciparum_orthologs_venn.svg", height = 7, width = 7, fix_text_size=FALSE, system_fonts = list(sans = "Open Sans"))
ggVennDiagram(ortholog_list,
  ...
  scale_fill_gradient(low="aliceblue",
                      high = "deepskyblue3", 
                      name="Number of Orthologs")
# dev.off()
```


```{r}
fit <- euler(combinations = ortholog_list,
             shape = "circle")
# head(fit)

# svg("figures/uncorrected/pvivax_vs_pfalciparum_orthologs_proportional_venn.svg",width = 7, height = 5, pointsize = 14)
# svglite::svglite("figures/uncorrected/pvivax_vs_pfalciparum_orthologs_proportional_venn.svg", height = 5, width = 7, fix_text_size=FALSE, system_fonts = list(sans = "Open Sans"), pointsize = 20)
par(mar = c(10,10,10,10))
plot(fit,
     fills = list(fill = c("turquoise4","#BBDF27FF"),
                  alpha = 0.7),
     ...
     legend = list(col = "black", alpha = 0.8, fontsize = 16))
# dev.off()
```
...
```{r}
orthologs_go_df <- read.csv("results/pvivax_vs_pfalciparum/orthologs_go_ClusterProfiler_AllOntology_01.17.2023.csv") %>% 
     mutate(neg_log10_q=-log10(qvalue),
            neg_log10_p=-log10(pvalue))

# write.csv(orthologs_go_df, "results/pvivax_vs_pfalciparum/orthologs_go_ClusterProfiler_AllOntology.csv", row.names = FALSE)
# table(orthologs_go_df$ortholog_expression_category)

dim(orthologs_go_df) # 1345   12
```
...
table(chk$ortholog_expression_category,chk$neg_log10_p > threshold) # 15 need to be removed
  
dim(chk) # 192

quantile_df <- function(x, probs = c(0, 0.5,  1)) {
 tibble(quantile = probs, value = round(quantile(x, probs), digits = 2))
}

chk %>% 
  filter(ontology != "N/A",
        qvalue < 0.2, neg_log10_p > threshold) %>%
  select(ortholog_expression_category, pvalue, p.adjust) %>% 
  group_by(ortholog_expression_category) %>%
  reframe(across(starts_with("p"), quantile_df, .unpack = TRUE))
```

```{r}
chk %>% 
  filter(ortholog_expression_category == 'pv_only') %>% 
  # select(go_term_name, pvalue, p.adjust) %>% 
  View()
```

```{r}
orthologs_go <- purrr::map(unique(orthologs_go_df$ortholog_expression_category) %>% set_names(., values=.), 
                           function(x) filter(orthologs_go_df, ortholog_expression_category == x))
...
p <- purrr::map(names(orthologs_go), function(group){
  ...
  df <- orthologs_go[[group]] %>% 
    ...
    filter(
            ontology != "N/A",
           qvalue < 0.2,
           # neg_log10_p > threshold
           ) %>%
    mutate(setSize=as.integer(setSize)) %>% 
    dplyr::arrange(pvalue)
  
    # svglite::svglite(paste0("figures/uncorrected/bubble_plots/ortholog_expression_",group,"_ClusterProfiler_bubblePlot.svg"),fix_text_size=FALSE, system_fonts = list(sans = "Open Sans"), height = 5, width = 8)
