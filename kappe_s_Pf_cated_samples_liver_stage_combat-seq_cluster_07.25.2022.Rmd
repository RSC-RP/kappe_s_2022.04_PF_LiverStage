...
day2 <- dge_lib.sizes %>% 
  ...
  mutate(infection_status = ifelse(is.na(infection_status),
         ...
         total_bp_sequenced = lib.size * read_length, 
...
# day2_counts <- pf_combat_dge$counts[,day2$sample_id]
# subset_day2_counts <- lapply(colnames(day2_counts), function(col){
#   df <-  data.frame(gene_name = rownames(day2_counts), 
#                     counts =  day2_counts[,col], 
#                     sample = col) %>% 
#     filter(counts > 0)
# })
# 
...
ggplot(day2, aes(y = infection_status, x = analysis_group_id)) +
  geom_tile(aes(fill = tx_coverage), color = "white") +
  geom_text(aes(label = tx_coverage_label)) +
  scale_fill_gradient(high = "red", low = "dodgerblue")  +
  labs(title = "Pf Transcriptome Coverage: Day 2 Samples") +
  theme_classic()
